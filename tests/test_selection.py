...
def test_exploration(trial_dataset_split):
    ...
    selected = exploration(
        candidates.data, decorated.data, "refractive_index", num_to_select
    )
    assert len(selected) == num_to_select
...
def test_extremise(trial_dataset_split):
    ...
    selected = extremise_expected_value(
        candidates.data, decorated.data, "band_gap", num_to_select=num_to_select
    )
    assert len(selected) == num_to_select
...
def test_rppf(trial_dataset_split):
    ...
    selected = rppf(
        candidates.data,
        decorated.data,
        {"band_gap": "max", "refractive_index": "max"},
        num_to_select=num_to_select,
    )
    assert len(selected) == num_to_select
...
def test_fom_leakage(trial_dataset_split):
    ...
    decorated, candidates = trial_dataset_split
    selected = fom_high_k_leakage(
        candidates.data, decorated.data, include_std=True, num_to_select=num_to_select
    )
    assert len(selected) == num_to_select
