...
structures_path = data_path / "alexandria_pbesol_structures.json"
moddata_path = Path(__file__).absolute().parent.parent / "data" / "mod.data_feature_selected.pkl"

with open(structures_path) as f:
    structures = json.load(f)["structures"]["_alexandria_band_gap >= 0.3 AND _alexandria_hull_distance <= 0.025"]["https://alexandria.icams.rub.de/pbesol"]["data"]

pmg_structures = []
targets = []

for ind, s in tqdm.tqdm(enumerate(structures)):
    pmg_structures.append(Structure(s).as_pymatgen)

moddata = MODData.load(moddata_path)
moddata.featurizer = MatminerFastFeaturizer()
moddata.featurizer.featurizer_mode = "single"
moddata.df_featurized = None

moddata.df_structure = pd.DataFrame({"id": list(range(len(pmg_structures))), "structure": pmg_structures})
moddata.df_structure.set_index("id", inplace=True)
